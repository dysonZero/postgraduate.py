...
class Decoder(nn.Module):
    def __init__(self, dim: int, heads: int, dropout: float, window_size: int):
        ...
        self.MHA = nn.ModuleList(
            [nn.MultiheadAttention(embed_dim=dim, num_heads=heads, dropout=dropout) for x in range(2)])
        self.FFN = nn.Sequential(
            nn.Linear(in_features=dim, out_features=dim, bias=True),
            nn.ReLU(),
            nn.Linear(in_features=dim, out_features=dim, bias=True),
...
class SAINT(nn.Module):
    def __init__(self, device, num_layers: tuple, num_skills: int, window_size: int, dim: int, heads: int,
                 dropout: float):
        ...
        self.Encoders = nn.ModuleList(
            [copy.deepcopy(Encoder(dim=dim, heads=heads, dropout=dropout, window_size=window_size)) for x in
             range(num_layers[0])])
        self.Decoders = nn.ModuleList(
            [copy.deepcopy(Decoder(dim=dim, heads=heads, dropout=dropout, window_size=window_size)) for x in
             range(num_layers[1])])
